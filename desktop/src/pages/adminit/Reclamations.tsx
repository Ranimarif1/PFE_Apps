...
import { getStatusBadgeClass, getStatusLabel, getStatusSurfaceClass, getActiveFilterTabClass, INACTIVE_TAB_CLASS } from "@/styles/statusSystem";
  ...
  return (
    <AppLayout title="Gestion des réclamations">
      <div className="space-y-4">
        {/* Filters */}
        <div className="flex gap-6 flex-wrap border-b border-border">
          {[
            { key: "tous", label: "Tous" },
            { key: "pending", label: getStatusLabel("pending", "complaint") },
            { key: "in_progress", label: getStatusLabel("in_progress", "complaint") },
            { key: "resolved", label: getStatusLabel("resolved", "complaint") },
          ].map(({ key, label }) => (
          ...
          <div key={r._id} className="bg-card rounded-xl border border-border shadow-card p-5">
            ...
            {r.response && (
              <div className={cn("mt-3 p-3", getStatusSurfaceClass("resolved"))}>
                <p className="text-xs font-semibold text-[#4D7F67] mb-1">Réponse envoyée :</p>
